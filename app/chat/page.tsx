...
export default function BharatJurisChat() {
  ...
  const handleDownloadChat = () => {
    ...
    let chatHtml = messages.map(m => {
        ...
        const cleanContent = m.content.replace(/\*\*(.*?)\*\*/g, '<b>$1</b>').replace(/\n/g, '<br/>');
        return `
    ...
    const header = `
      ...
      <head><meta charset='utf-8'><title>Legal Consultation History</title></head><body>
      <h1 style="text-align: center;">LEGAL CONSULTATION TRANSCRIPT</h1>
      <p style="text-align: center; color: #666;">Generated by BharatJuris on ${new Date().toLocaleDateString()}</p>
      <hr/>${chatHtml}<br/><hr/>
      <p style="text-align: center; font-size: 9pt; color: #888;">Disclaimer: Generated by AI. Not official legal advice.</p>
      </body></html>
    `;
  ...
  return (
    <div className="flex h-screen bg-[#050505] text-white font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col relative bg-[#050505] overflow-hidden">
        ...
        {/* HEADER */}
        <header className="h-16 border-b border-[#1f1f1f] flex items-center px-6 justify-between bg-[#0a0a0a]/50 backdrop-blur-md sticky top-0 z-50">
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                {isDropdownOpen && (
                    <div className="absolute right-0 top-full mt-2 w-48 bg-[#111] border border-[#333] rounded-xl shadow-xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                        {languages.map((lang) => (
                        <button key={lang.code} onClick={() => { setSelectedLang(lang); setIsDropdownOpen(false); }} className={`w-full text-left px-4 py-3 text-sm hover:bg-[#222] transition-colors flex items-center gap-3 ${selectedLang.code === lang.code ? 'bg-[#1a1a1a]' : ''}`}>
                            <span className={`flex items-center justify-center w-6 h-6 text-[10px] font-bold rounded-full ${selectedLang.code === lang.code ? 'bg-blue-600 text-white' : 'bg-[#333] text-gray-400'}`}>{lang.short}</span>
                            <span className={selectedLang.code === lang.code ? 'text-white font-medium' : 'text-gray-300'}>{lang.label}</span>
                        </button>
                        ))}
                    </div>
                )}
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 scrollbar-thin scrollbar-thumb-gray-800 z-10">
            {messages.map((m) => (
                <div key={m.id} className={`flex gap-4 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`max-w-[85%] p-4 rounded-2xl text-[15px] shadow-sm ${m.role === 'user' ? 'bg-[#2563eb] text-white rounded-br-none' : 'bg-[#1a1a1a] text-gray-200 border border-[#333] rounded-bl-none'}`}>
                        {m.role === 'user' ? (
            ...
            <div className="max-w-4xl mx-auto relative group">
                <form onSubmit={handleSubmit} className="relative flex items-center bg-[#111] border border-[#333] rounded-2xl focus-within:border-gray-600 focus-within:ring-1 focus-within:ring-gray-600 transition-all shadow-lg">
                    ...
                    <div className="absolute right-3 flex items-center gap-2">
                        <button type="button" onClick={handleVoiceInput} disabled={isTranscribing} className={`p-2 rounded-full transition-all ${isRecording ? 'bg-red-500 text-white animate-pulse' : isTranscribing ? 'bg-gray-800' : 'text-gray-400 hover:text-white hover:bg-gray-800'}`}>
                        ...
                        <button type="submit" disabled={isLoading || !input} className="bg-blue-600 text-white p-2.5 rounded-xl hover:bg-blue-500 disabled:opacity-50 transition-all shadow-md">
                            <Send className="w-4 h-4" />
                        </button>
                    </div>
                </form>
                <div className="text-center mt-3 flex justify-center items-center gap-2 opacity-60">
                    <p className="text-[10px] text-gray-500 uppercase tracking-widest font-medium">AI Generated • Verify with Article 39A</p>
                </div>
            </div>
        </div>
